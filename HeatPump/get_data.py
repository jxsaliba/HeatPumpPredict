...
def get_data():
    ...
    # Specify the dataframes and read each into a CSV
...
=======
    ...
    # change to datetime
        # Convert context_date column to datetime
    result['context_date'] = pd.to_datetime(result['context_date'], dayfirst = True)
    # Convert lodgement_date column to datetime
    result['lodgement_date'] = pd.to_datetime(result['lodgement_date'], dayfirst = True)

    # sort by property ID and context date
    result = result.sort_values(by=['property_id', 'context_date'])

>>>>>>> c9be20033472e84176a88a41e0b2634a4f4e94c2
    return result
