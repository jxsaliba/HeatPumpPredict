...
def train_test_arima_b(epc_b):
    ...
    df_b = epc_b[epc_b['property_id']==20238]
    y4 = df_b['energy_ma168'][start_point: end_point]
    exog4 = df_b['weather_temp_ma168'][start_point: end_point]

    train4, test4 = model_selection.train_test_split(y4, train_size=0.7)
    ...
    preds, conf_int = modl_b.predict(n_periods=test4.shape[0], return_conf_int=True)
...
def train_test_arima_c(epc_c):
    ...
    # Extract the relevant data for prediction
    df_c = epc_c[epc_c['property_id']==20378]
    y = df_c['energy_ma168'][start_point: end_point]
    exog = df_c['weather_temp_ma168'][start_point: end_point]

    train, test = model_selection.train_test_split(y, train_size=0.7)
    ...
    # Create predictions for the future, evaluate on test
    preds, conf_int = modl_c.predict(n_periods=test.shape[0], return_conf_int=True)
...
def train_test_arima_d(epc_d):
    ...
    # Extract the relevant data for prediction
    df_d = epc_d[epc_d['property_id']==20187]
    y2 = df_d['energy_ma168'][start_point: end_point]
    exog2 = df_d['weather_temp_ma168'][start_point: end_point]

    train2, test2 = model_selection.train_test_split(y2, train_size=0.7)
    ...
    # Create predictions for the future, evaluate on test
    preds, conf_int = modl_d.predict(n_periods=test2.shape[0], return_conf_int=True)
...
def train_test_arima_e(epc_e):
    ...
    # Extract the relevant data for prediction
    df_e = epc_e[epc_e['property_id']==20223]
    y3 = df_e['energy_ma168'][start_point: end_point]
    exog3 = df_e['weather_temp_ma168'][start_point: end_point]

    train3, test3 = model_selection.train_test_split(y3, train_size=0.7)
    ...
    # Create predictions for the future, evaluate on test
    preds, conf_int = modl_e.predict(n_periods=test3.shape[0], return_conf_int=True)
...
# pred_b, const_b = train_test_arima_b(epc_b)
# print(pred_b)
# print('---')

# pred_c, const_c = train_test_arima_c(epc_c)
# print(pred_c)
# print('---')

# pred_d, const_d = train_test_arima_d(epc_d)
# print(pred_d)
# print('---')

pred_e, const_b = train_test_arima_e(epc_e)
print(pred_e)
